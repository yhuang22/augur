...
Remark: This command may not work, and there is no need to be concerned.
```

3. Create a new file named `augur.config.json` and add your GitHub API key to designated position. (You may need to copy the content from `sample.config.json`.
...
make dev
```
To view the UI, use the link generated by the command `make dev`, it will resemble 'http://localhost:3333' to some extend.
